...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateCounter(element, target, duration = 2000) {
        ...
        const timer = setInterval(() => {
            ...
            // Formater le nombre (ajouter des séparateurs de milliers si nécessaire)
            if (target >= 1000) {
    ...
    function initCounters() {
        ...
        if (statNumbers.length === 0) {
            return; // Pas de compteurs trouvés
        }
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (target && !statNumber.classList.contains('animated')) {
                        ...
                        statsObserver.unobserve(statNumber); // Ne plus observer après l'animation
                    }
                }
            });
        }, {
        ...
        // Observer tous les compteurs statistiques
        statNumbers.forEach(stat => {
            // Vérifier si le compteur est déjà visible au chargement
            const rect = stat.getBoundingClientRect();
            const isVisible = rect.top < window.innerHeight && rect.bottom > 0;
            
            if (isVisible && !stat.classList.contains('animated')) {
                // Si visible, lancer l'animation immédiatement
                ...
                // Sinon, observer pour quand il devient visible
